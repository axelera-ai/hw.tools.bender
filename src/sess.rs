...
impl<'io, 'sess: 'io, 'ctx: 'sess> SessionIo<'sess, 'ctx> {
    ...
    pub async fn dependency_versions(
        ...
        match dep.source {
            ...
            DependencySource::Git(ref url) => self
                .git_database(&dep.name, url, force_fetch)
    ...
    async fn git_versions(&'io self, git: Git<'io, 'sess, 'ctx>) -> Result<GitVersions<'ctx>> {
        match self
            .git_versions
            .lock()
            .unwrap()
    ...
    async fn checkout_git(
        ...
        let scrapped = async { Ok(path.exists()) }
        ...
        let updated = scrapped.await.and_then(|_| {
            serde::__private::Ok(async move {
                if !path.exists() {
            ...
            (&DepSrc::Git(ref url), DepVer::Git(rev)) => {
                ...
                self.git_database(&dep.name, url, false)
                        .and_then(|db| async move {
                            db.list_files(rev, Some("Bender.yml"))
                                .and_then(|entries| async move {
                                    match entries.into_iter().next() {
                                        None => Ok(None),
                                        Some(entry) => {
                                            db.cat_file(entry.hash).and_then(|f| async move { Ok(Some(f))}).await
                                        }
                                    }
                                },
                            ).await
                        })
                        .and_then(|data| async move {match data {
                            Some(data) => {
        ...
        let manifests = join_all(
            self.sess
                ...
                .map(|pkgs| async move {
                    join_all(
                        pkgs.iter()
                            .map(|&pkg| async move { self.dependency_manifest(pkg) })
                            .collect::<Vec<_>>(),
                    )
                })
                .collect::<Vec<_>>(),
    ...
    pub async fn plugins(&'io self) -> Result<&'ctx Plugins> {
            ...
            self.sess
                ...
                .map(|pkgs| async move {
                    join_all(
                        pkgs.iter()
                            .map(|&pkg| async move {
                                self.dependency_manifest(pkg).map(move |m| (pkg, m))
                            })
                            .collect::<Vec<_>>(),
                    )
                })
        ...
        manifests
            .map(|manifests| async {
                ...
                for (package, manifest) in manifests.await {
