...
impl<'git, 'io, 'sess: 'io, 'ctx: 'sess> Git<'io, 'sess, 'ctx> {
    ...
    pub async fn fetch(self, remote: &str) -> Result<()> {
        ...
        let fetch1 = self.spawn_with(|c| c.arg("fetch").arg("--prune").arg(r1));
        // .and_then(|_|
        let fetch2 = self.spawn_with(|c| c.arg("fetch").arg("--tags").arg("--prune").arg(r2));
        // )
        // .await
        try_join!(fetch1, fetch2).map(|_| ())
    }
    ...
    pub async fn list_revs(self) -> Result<Vec<String>> {
        self.spawn_with(|c| c.arg("rev-list").arg("--all").arg("--date-order"))
            .and_then(|raw| async move { Ok(raw.lines().map(String::from).collect()) })
            .await
    }
    ...
    pub async fn current_checkout(self) -> Result<Option<String>> {
        self.spawn_with(|c| c.arg("rev-parse").arg("--revs-only").arg("HEAD^{commit}"))
            .and_then(|raw| async move { Ok(raw.lines().take(1).map(String::from).next()) })
            .await
    }
    ...
    pub async fn list_files<R: AsRef<OsStr>, P: AsRef<OsStr>>(
        ...
        .and_then(|raw| async move { Ok(raw.lines().map(TreeEntry::parse).collect()) })
        .await
    }
...
// pub type GitFuture<T> = impl Future<Output = Result<T>>;
